...
#define SPI_CR2_FRF               ((uint16_t)0x0000)                    ////////TIÄ£Ê½¹Ø±Õ
...
void SPI_TransmitReceive(SPI_TypeDef* SPIx,uint8_t *pTxData, uint8_t *pRxData,int16_t Size)
{
    ...
    {
      /* check TXE flag */
      if((txallowed==1U) &&(txcount > 0U) && (SPI_I2S_GetFlagStatus(SPI5, SPI_I2S_FLAG_TXE) == RESET))
      {
        *(__IO uint8_t *)&SPIx->DR = (*pTxData++);
        txcount--;
      ...
      if((rxcount > 0U) && (SPI_I2S_GetFlagStatus(SPI5, SPI_I2S_FLAG_RXNE) == RESET))
      {
        (*(uint8_t *)pRxData++) = SPIx->DR;
        rxcount--;
